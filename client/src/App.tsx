...
function App() {
        ...
        const handleJoinedRoom = (rid: string) => {
            ...
            if (rid !== 'lobby') {
                ...
                if (creatingSteamLobbyRef.current) {
                    ...
                    (async () => {
                        const hostEndpoint = await resolveSteamHostEndpoint();
                        if (!hostEndpoint) {
                            throw new Error('Unable to resolve host endpoint for Steam lobby');
                        }
                        return await steamService.createLobby(rid, 'Random', 'Standard', hostEndpoint);
                    })()
                        .then(res => {
                            if (res.success) {
    ...
    const resolveSteamHostEndpoint = async (): Promise<string | null> => {
                    ...
                    {menuView === 'statistics' && (
                        <div className="menu-column menu-column--statistics">
                            ...
                            <StatisticsPanel
                                ...
                                totalCampaignStages={CAMPAIGN_LEVELS.length}
